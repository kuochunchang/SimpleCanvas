	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				...
				//	Log.d(TAG, "message buffer "
				//			+ new String(buffer, 0, bytes));

					int semicolonPos = data.indexOf(";");
					if (semicolonPos > -1) {
						
						String response = data.substring(0, semicolonPos).trim();
						data = data.substring(semicolonPos+1, data.length()).trim();

						// Send the obtained bytes to the UI Activity

						mHandler.obtainMessage(MESSAGE_READ, bytes, -1, response)
								.sendToTarget();
						
						semicolonPos = data.indexOf(";");
						if (semicolonPos > -1) {
							response = data.substring(0, semicolonPos).trim();
							data = data.substring(semicolonPos+1, data.length()).trim();
							mHandler.obtainMessage(MESSAGE_READ, bytes, -1, response)
							.sendToTarget();
						}
